...
export default function OrderCard({ order } : OrderCardProps) {
    ...
    return (
        ...
        >
            <p className='text-2xl font-medium text-gray-900'>Cliente: <span className="font-bold">{order.name}</span></p>
            <p className='text-lg font-medium text-gray-900'>Productos Ordenados:</p>
            <dl className="mt-6 space-y-4">
                {order.orderProducts.map( product => (
                    <div 
                        key={product.id} 
                        className="flex item-center gap-2 border-t border-gray-200 pt-4"
                    >
                        <dt className="flex item-center text-sm text-gray-600">
